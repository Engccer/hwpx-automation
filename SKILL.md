...
│   └── HWP/스캔 PDF/이미지 → Upstage DocParse (이미지 기반 문서 인식, API 필요)
│         python C:/Users/pc/Converters/DocParse/upstage_parse.py <파일>
├── 편집 (기존 문서 수정)
...
2. **Blockquote 전처리**: MD에서 `> ` 줄을 `【인용】` 마커로 치환 (hwpx_convert.py가 blockquote 내용을 누락시킴)
...
> Pandoc 방식의 한계 (blockquote 누락, 각주 미변환, 제한적 스타일)를 완전히 해결.
