...
17. **편집 명령의 저장 경로에 조건을 걸지 말 것** (2026-08-12 리뷰 지적, 수정 완료): `cmd_find_replace`가 표 치환이 1건 이상일 때만 `save_hwpx()`를 타도록 돼 있어, **본문에만 매칭된 흔한 경우**에는 python-hwpx 산출물이 그대로 나가고 `sanitize_header`(검은 배경)·`fix_empty_cells`·ZIP 레이아웃 복제가 통째로 건너뛰어졌다. 같은 명령이 "표에 걸렸는가"에 따라 다른 품질을 내는 상태였다. 지금은 표 치환이 0건이어도 항상 `save_hwpx()`를 탄다. **새 편집 명령을 만들 때도 저장은 무조건 `save_hwpx()` 한 경로로 모을 것**(15번의 규칙과 한 쌍).
18. **hwp2hwpx 변환본의 XML 1.0 불법 제어문자로 파싱 실패** (2026-08-20 실측, 수정 완료): `hwp2hwpx-1.0.0.jar`는 HWP 하이퍼링크 필드의 `Command` 문자열을 NUL 패딩째로 `<hp:stringParam>`에 써 넣는다. NUL은 XML 1.0에서 이스케이프로도 표현할 수 없어 lxml이 `Premature end of data in tag stringParam`으로 파싱을 중단했고, `--info`·`--set-cell` 등 lxml 경로와 `--find/--replace`의 python-hwpx 경로가 **모두** raw 트레이스백으로 죽었다.
    ...
    - **`--to-md`는 엔진 소관**: hwpx-tomd 0.2.1이 같은 방어를 갖는다(이슈 hwpx-tomd#1). PyPI 배포 후 `requirements.txt` 핀을 `>=0.2.1`로 올려야 새로 설치하는 사용자에게도 참이 된다.
    - **새 코드 규칙**: HWPX를 읽는 새 경로를 만들면 `zipfile`로 읽은 직후 `sanitize_xml_parts()`를 태운다(15번의 `write_hwpx_zip`·`serialize_xml` 규칙과 같은 계열).
