...
## 2026-06-10 · 고사 원안 2단 인쇄본 "스타일 충실" 자동 조판 (실패 진단 → v2 빌더)

작업 맥락: 2026 1학년 기말 원안.md → 인쇄 hwpx 1차 산출물(v1, python-hwpx 양식 채우기)이 시각 품질 미달로 폐기됨(표 내부 정렬·문항 간 빈 줄·들여쓰기·박스 여백 전부 어긋남). 레퍼런스(2025 영어과 최종본 hwpx)와 XML 레벨로 대조 진단 후 v2 빌더로 재생성, 7면 산출(레퍼런스와 동일 분량)·recall 무손실.

### 교훈 14 — "XML 코딩이라 스타일이 깨진다"가 아니라 "조판 어휘를 안 썼다"가 원인

- **진단(정량)**: v1 산출물은 본문 문단 전체가 단일 paraPr 37(발문용)·charPr 0, 표는 기본 borderFill·셀 여백 0·전부 CENTER, md 빈 줄 전부 폐기. 반면 레퍼런스 최종본은 블록 타입별 paraPr 체계를 쓴다 — 발문(내어쓰기 -1520·줄간격 160), 선택지(-1100·150), 대화 줄(-3452·150 + 화자 뒤 탭), 박스 셀(여백 510/510/141/141·테두리 0.1mm), 데이터 표(0.12mm), 문항 사이 빈 문단 2~10개(단 끝 밀어내기).
- **해법(v2)**: 레퍼런스에서 추출한 조판 명세를 양식 header.xml에 paraPr 9종·borderFill 2종으로 주입(itemCnt 갱신 필수)하고, md 블록 타입(발문/선택지/세트/대화/산문/보기/서답형/표)별로 해당 스타일을 적용. 같은 XML 경로로도 레퍼런스와 동급의 시각 품질이 나온다.
- **상태**: 기록 (고사문항출제 스킬 `레이아웃-조판-예산.md`에 명세 정본 반영)

### 교훈 15 — 대화 박스 = 1×1 표 + 화자 뒤 <hp:tab/> + 행잉 인덴트가 안전한 재현

- **레퍼런스 실물**: 대화 지문은 고정 크기 글상자(hp:rect/drawText) 안에 `<hp:t>화자: <hp:tab/>발화</hp:t>` + paraPr 내어쓰기 -3452. 글상자는 높이가 고정이라 자동 생성에 부적합(내용량 변화에 깨짐).
- **재현**: 1×1 표(테두리 0.1mm = 글상자와 동일 룩, 높이 자동)에 같은 문단 구조를 넣으면 시각 동일 + 내용량 안전. 탭은 python-hwpx가 못 넣으므로 본문에 PUA 마커(U+E001)를 넣고 저장 후 section0.xml에서 `<hp:tab width="292" leader="0" type="1"/>`로 치환(탭·줄바꿈은 hp:t 안에 중첩되는 요소). 두 줄 선택지(대화형)는 `<hp:lineBreak/>`(U+E004 마커)로 같은 문단 안에서 줄바꿈하면 행잉 인덴트가 둘째 줄을 번호 뒤로 정렬해 준다.
- **상태**: 기록

### 교훈 16 — md의 면/단 계획을 pageBreak·columnBreak로 강제하면 자연 흐름과 충돌해 페이지가 산개

- **증상**: 원안.md의 `PAGE n·좌단/우단` 주석을 hp:p pageBreak/columnBreak 속성으로 강제했더니, 계획보다 내용이 길어진 단에서 잔여 내용이 다음 단으로 흐른 뒤 또 나눔이 걸려 6면 계획이 9면으로 산개.
- **해법**: 자동 조판 단계에서는 강제 나눔 없이 자연 흐름으로 두고(빈 문단 리듬만 유지), 면 배분(48행 균형)·`☞ 뒷면 계속` 삽입은 한글에서 최종 수동 단계로 남긴다. 레퍼런스 작성자도 빈 문단 2~10개로 수동 밀어내기를 했다.
- **상태**: 기록

### 교훈 17 — COM 마무리: --normalize(열어서 재저장)로 lineseg·미리보기를 한컴 네이티브로

- **활용**: XML 빌더 산출물을 hwpx_com.py `--normalize`로 한컴이 직접 재저장하게 하면 lineseg·PrvText/PrvImage·내부 캐시가 모두 정식 재계산된다(본문 자수 보존 검증 포함). 인쇄·배포 직전 표준 마무리.
- **함정(재확인)**: 같은 프로세스에서 pyhwpx Hwp 인스턴스를 두 번 만들면 "개체가 서버에 연결되지 않았습니다" COM 오류 — save_as 후 같은 인스턴스로 출력본을 다시 열어 검증해야 한다(주의사항 2번과 동일 뿌리).
- **상태**: **도구 반영됨** (`hwpx_com.py --normalize`, 2026-06-10)
